...
import { type CoinStruct, type IotaClient } from '@iota/iota-sdk/client';
...
async function getCoinsByBalance({
    ...
    const bigIntBalance = BigInt(
        new BigNumber(balance).integerValue(BigNumber.ROUND_UP).toString(),
    );
...
function getBalanceAndWalletFees(balance: string, totalBalance: string, conversionRate: number) {
    ...
    if (balanceAndWalletFees.isGreaterThan(bigNumberTotalBalance)) {
         ...
         */
        const remainingBalance = bigNumberBalance
            .minus(new BigNumber(walletFees).times(2))
            .toString();
