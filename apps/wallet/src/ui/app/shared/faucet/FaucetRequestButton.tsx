...
import { FaucetRateLimitError, getFaucetHost } from '@iota/iota-sdk/faucet';
...
function FaucetRequestButton({ variant = 'primary', size = 'narrow' }: FaucetRequestButtonProps) {
    ...
    const networkName = API_ENV_TO_INFO[network].name.replace(/iota\s*/gi, '');
    ...
    return mutation.enabled ? (
        <Button
            ...
            text={`Request ${networkName} IOTA Tokens`}
        />
    ) : null;
}
