...
mod tests {
    use iota::{
        client_commands::{IotaClientCommandResult, IotaClientCommands},
    ...
    async fn test_discard_invalid_gas() {
        ...
        let res = IotaClientCommands::PayAllIota {
        ...
        if let IotaClientCommandResult::PayAllIota(response) = res {
            assert!(matches!(
                response.effects.unwrap().status(),
                IotaExecutionStatus::Success
            ));
        } else {
            panic!("PayAllIota command did not return IotaClientCommandResult::PayAllIota");
        };
    ...
    async fn test_discard_smaller_amount_gas() {
        ...
        let res = IotaClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
        ...
        let tiny_coin_id = if let IotaClientCommandResult::SplitCoin(resp) = res.unwrap() {
            resp.effects.as_ref().unwrap().created()[0]
                .reference
                .object_id
        } else {
            panic!("split command did not return IotaClientCommandResult::SplitCoin");
        };
    ...
    async fn test_faucet_no_loop_forever() {
        ...
        let tiny_value = (config.num_coins as u64 * config.amount) + 1;
        let _res = IotaClientCommands::SplitCoin {
            coin_id: *gases[0].id(),
            amounts: Some(vec![tiny_value]),
            gas_budget: 50000000,
            gas: None,
    ...
    async fn get_current_gases(address: IotaAddress, context: &mut WalletContext) -> Vec<GasCoin> {
        // Get the latest list of gas
        let results = IotaClientCommands::Gas {
        ...
        .unwrap();
        match results {
            IotaClientCommandResult::Gas(gases) => gases,
            other => panic!("Expect IotaClientCommandResult::Gas, but got {:?}", other),
        }
    }
}
