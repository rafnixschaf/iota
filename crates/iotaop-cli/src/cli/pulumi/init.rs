...
impl ProjectType {
    pub fn create_project(&self, use_kms: &bool, project_name: Option<String>) -> Result<()> {
        ...
        let project_subdir = match self {
            ...
            Self::Basic => Text::new("project subdir (under iota-operations/pulumi/):")
                .prompt()
...
fn ensure_in_iotaops_repo() -> Result<()> {
    ...
    .stdout;
    let in_iotaops = String::from_utf8_lossy(&remote_stdout)
        .trim()
        .contains("iota-operations");
    if !in_iotaops {
        Err(anyhow!(
