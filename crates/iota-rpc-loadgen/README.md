...
To try this locally, refer to the [docs](https://wiki.iota.org/guides/developer/getting-started/local-network). Recommend setting `database-url` to an env variable. Note: run `RUST_LOG="consensus=off" cargo run iota --features indexer -- start --with-faucet --force-regenesis --with-indexer` to rebuild.
