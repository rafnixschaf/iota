...
/// The names of the minimum metrics exposed by the load generators that are
/// required to compute performance.
pub trait ProtocolMetrics {
    ...
    /// The name of the metric reporting the total number of finalized
    /// transactions
    const TOTAL_TRANSACTIONS: &'static str;
    /// The name of the metric reporting the latency buckets.
    const LATENCY_BUCKETS: &'static str;
    /// The name of the metric reporting the sum of the end-to-end latency of
    /// all finalized transactions.
    const LATENCY_SUM: &'static str;
    /// The name of the metric reporting the square of the sum of the end-to-end
    /// latency of all finalized transactions.
