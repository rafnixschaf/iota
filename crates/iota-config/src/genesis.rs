...
use iota_types::deny_list::{get_coin_deny_list, PerTypeDenyList};
...
impl TokenDistributionSchedule {
    pub fn validate(&self) {
        ...
        if total_micros != TOTAL_SUPPLY_MICROS {
            panic!("TokenDistributionSchedule adds up to {total_micros} and not expected {TOTAL_SUPPLY_MICROS}");
        }
    }
