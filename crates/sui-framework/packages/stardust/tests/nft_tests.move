...
module stardust::nft_tests {
    ...
    fun nft_assets_extraction() {
        ...
        // Extract assets.
        let (iota, nft) = nft_output.extract_assets(scenario.ctx());
        ...
        assert!(nft.immutable_metadata().version() == string::utf8(b"0.0.1"), 5);
        assert!(nft.immutable_metadata().media_type() == string::utf8(b"image/png"), 6);
        assert!(nft.immutable_metadata().uri() == url::new_unsafe(ascii::string(b"www.best-nft.com/nft.png")), 7);
        assert!(nft.immutable_metadata().name() == string::utf8(b"nft"), 8);
        assert!(nft.immutable_metadata().collection_name().contains(&string::utf8(b"collection")), 9);
        assert!(nft.immutable_metadata().royalties().length() == 1, 10);
        assert!(nft.immutable_metadata().royalties()[sender] == fixed_point32::create_from_rational(1, 2), 11);
        assert!(nft.immutable_metadata().issuer_name().contains(&string::utf8(b"issuer")), 12);
        assert!(nft.immutable_metadata().description().contains(&string::utf8(b"description")), 13);
        ...
        assert!(returned_storage_deposit.value() == 1000, 16);
