...
module sui::big_coin {
    use std::string;
    use std::ascii;
    use sui::big_balance::{Self, BigBalance, BigSupply};
    use sui::url::{Self, Url};
    use sui::deny_list::{Self, DenyList};
    use std::type_name;

    // Allows calling `.split_vec(amounts, ctx)` on `coin`
    public use fun sui::big_pay::split_vec as BigCoin.split_vec;

    // Allows calling `.join_vec(coins)` on `coin`
    public use fun sui::big_pay::join_vec as BigCoin.join_vec;

    // Allows calling `.split_and_transfer(amount, recipient, ctx)` on `coin`
    public use fun sui::big_pay::split_and_transfer as BigCoin.split_and_transfer;

    // Allows calling `.divide_and_keep(n, ctx)` on `coin`
    public use fun sui::big_pay::divide_and_keep as BigCoin.divide_and_keep;
    ...
    public struct BigCoin<phantom T> has key, store {
        id: UID,
        balance: BigBalance<T>
    }
    ...
    public fun total_supply<T>(cap: &TreasuryCap<T>): u256 {
        big_balance::supply_value(&cap.total_supply)
    }

    /// Unwrap `TreasuryCap` getting the `BigSupply`.
    ///
    /// Operation is irreversible. Supply cannot be converted into a `TreasuryCap` due
    /// to different security guarantees (TreasuryCap can be created only once for a type)
    public fun treasury_into_supply<T>(treasury: TreasuryCap<T>): BigSupply<T> {
        let TreasuryCap { id, total_supply } = treasury;
        id.delete();
        total_supply
    }

    /// Get immutable reference to the treasury's `BigSupply`.
    public fun supply_immut<T>(treasury: &TreasuryCap<T>): &BigSupply<T> {
        &treasury.total_supply
    }

    /// Get mutable reference to the treasury's `BigSupply`.
    public fun supply_mut<T>(treasury: &mut TreasuryCap<T>): &mut BigSupply<T> {
    ...
    public fun value<T>(self: &BigCoin<T>): u256 {
        self.balance.value()
    }

    /// Get immutable reference to the balance of a coin.
    public fun balance<T>(coin: &BigCoin<T>): &BigBalance<T> {
        &coin.balance
    }

    /// Get a mutable reference to the balance of a coin.
    public fun balance_mut<T>(coin: &mut BigCoin<T>): &mut BigBalance<T> {
        &mut coin.balance
    }

    /// Wrap a balance into a Coin to make it transferable.
    public fun from_balance<T>(balance: BigBalance<T>, ctx: &mut TxContext): BigCoin<T> {
        BigCoin { id: object::new(ctx), balance }
    }

    /// Destruct a Coin wrapper and keep the balance.
    public fun into_balance<T>(coin: BigCoin<T>): BigBalance<T> {
        let BigCoin { id, balance } = coin;
        id.delete();
        balance
    }
    ...
    public fun take<T>(
        balance: &mut BigBalance<T>, value: u256, ctx: &mut TxContext,
    ): BigCoin<T> {
        BigCoin {
            id: object::new(ctx),
    ...
    public fun put<T>(balance: &mut BigBalance<T>, coin: BigCoin<T>) {
    ...
    public entry fun join<T>(self: &mut BigCoin<T>, c: BigCoin<T>) {
        let BigCoin { id, balance } = c;
        id.delete();
    ...
    public fun split<T>(
        self: &mut BigCoin<T>, split_amount: u256, ctx: &mut TxContext
    ): BigCoin<T> {
    ...
    public fun divide_into_n<T>(
        self: &mut BigCoin<T>, n: u64, ctx: &mut TxContext
    ): vector<BigCoin<T>> {
        assert!(n > 0, EInvalidArg);
        assert!((n as u256) <= value(self), ENotEnough);

        let mut vec = vector[];
        let mut i = 0;
        let split_amount = value(self) / (n as u256);
        while (i < n - 1) {
    ...
    public fun zero<T>(ctx: &mut TxContext): BigCoin<T> {
        BigCoin { id: object::new(ctx), balance: big_balance::zero() }
    }

    /// Destroy a coin with value zero
    public fun destroy_zero<T>(c: BigCoin<T>) {
        let BigCoin { id, balance } = c;
        id.delete();
    ...
    public fun create_currency<T: drop>(
        ...
        (
            TreasuryCap {
                id: object::new(ctx),
                total_supply: big_balance::create_supply(witness)
            },
            CoinMetadata {
                id: object::new(ctx),
                decimals,
    ...
    /// in `cap` accordingly.
    public fun mint<T>(
        cap: &mut TreasuryCap<T>, value: u256, ctx: &mut TxContext,
    ): BigCoin<T> {
        BigCoin {
            id: object::new(ctx),
    ...
    public fun mint_balance<T>(
        cap: &mut TreasuryCap<T>, value: u256
    ): BigBalance<T> {
    ...
    /// accordingly.
    public entry fun burn<T>(cap: &mut TreasuryCap<T>, c: BigCoin<T>): u256 {
        let BigCoin { id, balance } = c;
        id.delete();
    ...
    public entry fun mint_and_transfer<T>(
        c: &mut TreasuryCap<T>, amount: u256, recipient: address, ctx: &mut TxContext
    ) {
    ...
    public fun mint_for_testing<T>(value: u64, ctx: &mut TxContext): BigCoin<T> {
        BigCoin { id: object::new(ctx), balance: big_balance::create_for_testing(value) }
    }

    #[test_only]
    /// Burn coins of any type for testing purposes only
    public fun burn_for_testing<T>(coin: BigCoin<T>): u256 {
        let BigCoin { id, balance } = coin;
        id.delete();
    ...
    public fun create_treasury_cap_for_testing<T>(
        ...
        TreasuryCap {
            id: object::new(ctx),
            total_supply: big_balance::create_supply_for_testing()
        }
    }
}
