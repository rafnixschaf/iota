...
module sui::big_pay {
    use sui::big_coin::BigCoin as Coin;
    ...
    public entry fun split<T>(
        coin: &mut Coin<T>, split_amount: u256, ctx: &mut TxContext
    ) {
    ...
    public entry fun split_vec<T>(
        self: &mut Coin<T>, split_amounts: vector<u256>, ctx: &mut TxContext
    ) {
    ...
    public entry fun split_and_transfer<T>(
        c: &mut Coin<T>, amount: u256, recipient: address, ctx: &mut TxContext
    ) {
