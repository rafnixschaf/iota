...
// if you use the iota-test-validator and use the local network; if it does not work, try with port 5003.
...
/// Return the coin owned by the address that has at least 5_000_000 MICROS, otherwise returns None
...
pub async fn split_coin_digest(
    ...
    // first, we want to split the coin, and we specify how much IOTA (in MICROS) we want
    // for the new coin
