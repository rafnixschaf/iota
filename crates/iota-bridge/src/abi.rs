...
use crate::types::{BridgeAction, EthToIotaBridgeAction};
                ...
                match event {
                    EthIotaBridgeEvents::TokensBridgedToIotaFilter(event) => {
                        let bridge_event = match EthToIotaTokenBridgeV1::try_from(&event) {
                            ...
                            Err(e) => {
                                tracing::error!(?eth_tx_hash, eth_event_index, "Failed to convert TokensBridgedToIota log to EthToIotaTokenBridgeV1. This indicates incorrect parameters or a bug in the code: {:?}. Err: {:?}", event, e);
                                return None;
                            }
                        };
