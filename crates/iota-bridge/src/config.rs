...
pub struct BridgeNodeConfig {
    ...
    /// Path of the file where bridge client key (any IotaKeyPair) is stored as Base64 encoded `flag || privkey`.
    ...
    /// The iota modules of bridge packages for client to watch for. Need to contain at least one item when `run_client` is true.
    ...
    /// Override the last processed EventID for each bridge module. Key must be in `iota_bridge_modules`.
