...
fn test_basic_args_linter_top_level() {
    let path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../iota_programmability/examples/nfts");
    ...
    let path =
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../iota_programmability/examples/basics");
    let compiled_modules = BuildConfig::new_for_testing()
        .build(path)
        .unwrap()
        .into_modules();
    ...
    let path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("../iota-core/src/unit_tests/data/entry_point_vector");
    let compiled_modules = BuildConfig::new_for_testing()
        .build(path)
        .unwrap()
        .into_modules();
    ...
    let args = vec![IotaJsonValue::new(Value::Array(vec![object_id1, object_id2])).unwrap()];
