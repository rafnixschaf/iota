...
impl ProtocolConfig {
    ...
    fn get_for_version_impl(version: ProtocolVersion, chain: Chain) -> Self {
        ...
        let mut cfg = Self {
            ...
            consensus_max_transactions_in_block_bytes: Some(6 * 1_024 * 1024),
    ...
    fn lookup_by_string_test() {
        ...
        assert!(
            prot.lookup_attr("max_move_identifier_len".to_string())
                == Some(ProtocolConfigValue::u64(128))
        );
        ...
        assert!(
            prot.feature_flags
                .attr_map()
                .get("enable_group_ops_native_functions")
                .unwrap()
                == &true
        );
