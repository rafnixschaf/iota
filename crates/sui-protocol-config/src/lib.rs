...
// safemode. Version 3: gas model v2, including all sui conservation fixes. Fix
...
// 16_000_000. sui-system            framework changes.
...
// NW Batch to use versioned metadata field.             Changes to sui-system
...
// after consensus.             Add `sui::table_vec::drop` to the framework via
...
// `base_tx_cost_fixed`. Version 19: Changes to sui-system package to enable
...
// Version 25: Add sui::table_vec::swap and sui::table_vec::swap_remove to
// system packages. Version 26: New gas model version.
//             Add support for receiving objects off of other objects in devnet
// only. Version 28: Add sui::zklogin::verify_zklogin_id and related functions
// to sui framework.             Enable transaction effects v2 in devnet.
// Version 29: Add verify_legacy_zklogin_address flag to sui framework, this add
...
// referenced by receiving object in sui framework.             Create new
// execution layer version, and preserve previous behavior in v1.
// Update semantics of `sui::transfer::receive` and add
// `sui::transfer::public_receive`. Version 32: Add delete functions for
// VerifiedID and VerifiedIssuer.             Add sui::token module to sui
...
// (without msm). Version 42: Migrate sui framework and related code to Move
// 2024
...
/// The `sui-node` binary will always use the SYSTEM_DEFAULT constant, but for
/// testing we need to be able to inject arbitrary versions into SuiNode.
...
pub struct ProtocolConfig {
    ...
    /// Enforce by the Sui adapter. This is the sum of the serialized size
    ...
    /// completion. Enforce by the Sui adapter. Similar to
    ...
    /// the Sui adapter when effects are produced.
    ...
    /// Maximum size of a Move package object, in bytes. Enforced by the Sui
    ...
    /// Enforced by the Sui adapter.
    max_tx_gas: Option<u64>,

    /// Maximum amount of the proposed gas price in MIST (defined in the
    /// transaction).
    ...
    /// Per-byte cost of storing an object in the Sui global object store. Some
    /// of this cost may be refundable if the object is later freed
    obj_data_cost_refundable: Option<u64>,

    // Per-byte cost of storing an object in the Sui transaction log (e.g., in
