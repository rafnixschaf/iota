...
/// successfully. It also updates the treasury cap of a coin used for testing
/// which gets updated between transaction blocks if coins get minted. We need
/// this to work around limitations of the proptest framework where no external
/// mutable state can be used to the `prop_compose` macro.
pub fn run_pt_success(
    ...
    // at this point we know that the whole block executed successfully and we also
    // know that we are not properly dropping all the used values (in particular
    // empty vectors of coins generated as input - doing so for as many getter
