...
use iota_sdk::error::Error;

use iota_sdk::rpc_types::{IotaObjectDataOptions, IotaRawData, IotaRawMoveObject, IotaRawMovePackage};
...
impl<'a> BytecodeSourceVerifier<'a> {
    ...
    /// the bytecode at the address specified on the Iota network we are publishing to.
    ...
    /// at the address specified on the Iota network we are publishing to.  If `verify_deps` is true,
...
fn download_and_compile(
    ...
    if !dest_canonical_binary.exists() {
        ...
        let mainnet_url = format!(
            "https://github.com/iotaledger/kinesis/releases/download/mainnet-v{compiler_version}/iota-mainnet-v{compiler_version}-{platform}.tgz",
        );
        ...
        let mut response = match ureq::get(&mainnet_url).call() {
            ...
            Err(ureq::Error::Status(404, _)) => {
                ...
                let testnet_url = format!("https://github.com/iotaledger/kinesis/releases/download/testnet-v{compiler_version}/iota-testnet-v{compiler_version}-{platform}.tgz");
