...
///
/// This is intended as the only conversion function to go from Stardust to Iota addresses, so there is only
...
/// Converts a ["Stardust" `Address`](Address) to a [`IotaAddress`] and then wraps it into an [`Owner`]
