...
use iota_types::digests::{ConsensusCommitDigest, ObjectDigest, TransactionEventsDigest};
...
use iota_types::type_resolver::LayoutResolver;
        ...
        for tx_sig in &self.tx_signatures {
            builder.push_record(vec![format!(
                ...
                match tx_sig {
                    ...
                    _ => Base64::from_bytes(tx_sig.as_ref()).encoded(), // the signatures for multisig and zklogin are not iotated to be parsed out. they should be interpreted as a whole
                }
            )]);
        }
