...
impl IndexStore {
    ...
    /// 1. Fetches the next sequence number for the transaction and initializes
    ///    a batch operation for efficient database writes.
    /// 2. Updates multiple indexes in the `IndexStore`, including:
    ///    - Transaction order and sequence numbers.
    ///    - Transactions initiated by the sender and transactions related to
    ///      input and mutated objects.
    ///    - Transactions involving specific Move function calls.
    ///    - Transactions that resulted in objects being sent to specific
    ///      addresses.
    /// 3. Updates the coin index with changes related to coin transactions, and
    ///    handles any object index changes, such as owners and dynamic fields.
    /// 4. Indexes events generated by the transaction, including ordering,
    ///    modules, sender, Move events, and timestamps.
    /// 5. Updates the table of loaded child object versions based on the
    ///    transaction's effects.
    /// 6. Optionally invalidates caches before writing to the database to
    ///    ensure that the latest values are served.
    /// 7. Writes the batch of updates to the database, ensuring consistency
    ///    across all indices.
    /// 8. If caches are not invalidated before the write, updates them after
    ///    writing to ensure consistency with the indexed data.
