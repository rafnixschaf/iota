...
impl WritePathPendingTransactionLog {
    ...
    // 2. it does not race with the insert (`write_pending_transaction_maybe`) in a way that we
    //    care. 2.a. for one transaction, `finish_transaction` shouldn't predate
    //    `write_pending_transaction_maybe`. 2.b  for concurrent requests of one transaction, a call
    //    to this function may happen in between hence making the second request thinks it is the
    //    first record. It's preventable by checking this transaction again after the call of
