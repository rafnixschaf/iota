...
mod checked {
    ...
    use iota_types::{execution_mode::ExecutionMode, execution_status::CommandArgumentError};
    ...
    impl<'vm, 'state, 'a> ExecutionContext<'vm, 'state, 'a> {
        ...
        pub fn finish<Mode: ExecutionMode>(self) -> Result<ExecutionResults, ExecutionError> {
            ...
            // Iota Move programs should never touch global state, so resources should be empty
            assert_invariant!(
                change_set.resources().next().is_none(),
                "Change set must be empty"
            );
            // Iota Move no longer uses Move's internal event system
