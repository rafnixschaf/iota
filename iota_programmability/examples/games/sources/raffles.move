...
/// Basic raffles games that depends on Iota randomness.
///
...
module games::raffle_with_tickets {
    ...
    public fun buy_ticket(game: &mut Game, coin: Coin<IOTA>, clock: &Clock, ctx: &mut TxContext): Ticket {
        assert!(game.end_time > clock::timestamp_ms(clock), EGameAlreadyCompleted);
        assert!(coin::value(&coin) == game.cost_in_iota, EInvalidAmount);
    ...
    #[test_only]
    public fun get_cost_in_iota(game: &Game): u64 {
        game.cost_in_iota
    }
