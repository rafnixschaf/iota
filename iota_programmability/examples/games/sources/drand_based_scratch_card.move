...
///
/// Anyone can create a new game by depositing X*100 IOTAs as a reward, and setting the current drand round as the base
...
/// - Call buy_ticket() with the right amount of IOTA.
...
module games::drand_based_scratch_card {
    ...
    /// Buy a ticket for a specific game, costing reward/reward_factor IOTA. Can be called only during the epoch in which
    /// the game was created.
    /// Note that the reward might have been withdrawn already. It's the user's responsibility to verify that.
    public entry fun buy_ticket(coin: Coin<IOTA>, game: &Game, ctx: &mut TxContext) {
