...
/// An execution context that remaps the modules referred to at runtime
/// according to a linkage table, allowing the same module in storage to be run
/// against different dependencies.
///
/// Default implementation does no re-linking (Module IDs are unchanged by
/// relocation and the link context is a constant value).
pub trait LinkageResolver {
    ...
    /// `ModuleId` that originally defined that type.
    fn defining_module(
        &self,
        module_id: &ModuleId,
        _struct: &IdentStr,
