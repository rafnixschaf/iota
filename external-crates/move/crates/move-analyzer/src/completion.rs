...
/// Handle context-specific auto-completion requests with lbrace (`{`) trigger
/// character.
...
fn name_chain_completions(
    ...
    if leading_name.loc.contains(&cursor.loc) {
        ...
        // correct auto-completion in the first position
...
fn context_specific_no_trigger(
    ...
    // at this point only try to auto-complete init function declararation - get the
    // last string and see if it represents the beginning of init function
    // declaration
...
/// Return completion items, plus a flag indicating if we should only use the
/// custom items returned (i.e., when the flag is false, default items and
/// identifiers should also be added).
...
fn default_items(
    ...
    // or `::`, offer them context-specific autocompletion items as well as
