...
///   queue may contain any arbitrary type (such as a tuple of a block and expression), so the work
///   pattern is used to disassemble and bind component parts.
...
///   `$value_stack` (in that order). These values should always be available as per the contract of
///   the macro and how it disassembles and pushes values across its computation.
///
