...
// ***************************************************************************
//
// If `check_datatype_layout` and/or `check_datatype_and_pub_function_linking`
// is false, type safety over a series of upgrades cannot be guaranteed for
// either structs or enums. This is because the type could first be removed, and
// then re-introduced with a different layout and/or additional variants in a
// later upgrade. E.g.,
// * For enums you could add a new variant even if `disallow_new_variants` is
//   true, by first removing the enum in an upgrade, and then reintroducing it
//   with a new variant in a later upgrade.
// * For structs you could remove a field from a struct and/or add another field
//   by first removing removing the struct in an upgrade and then reintroducing
//   it with a different layout in a later upgrade.
...
/// `{ check_datatype_and_pub_function_linking: true, check_datatype_layout:
/// true, check_friend_linking: true, check_private_entry_linking: true }`:
/// fully backward compatible `{ check_datatype_and_pub_function_linking: true,
/// check_datatype_layout: true, check_friend_linking: true,
/// check_private_entry_linking: false }`: Backwards compatible, private entry
/// function signatures can change `{ check_datatype_and_pub_function_linking:
/// true, check_datatype_layout: true, check_friend_linking: false,
/// check_private_entry_linking: true }`: Backward compatible, exclude the
/// friend module declare and friend functions
/// `{ check_datatype_and_pub_function_linking: true, check_datatype_layout:
/// true, check_friend_linking: false, check_private_entry_linking: false }`:
/// Backward compatible, exclude the friend module declarations, friend
/// functions, and private and friend entry function
/// `{ check_datatype_and_pub_function_linking: false, check_datatype_layout:
/// true, check_friend_linking: false, check_private_entry_linking: _ }`:
/// Dependent modules that reference functions or types in this module may not
/// link. However, fixing, recompiling, and redeploying all dependent modules
/// will work--no data migration needed.
/// `{ check_datatype_and_pub_function_linking: true, check_datatype_layout:
/// false, check_friend_linking: true, check_private_entry_linking: _ }`:
/// Attempting to read structs published by this module will now fail at
/// runtime. However, dependent modules will continue to link. Requires data
/// migration, but no changes to dependent modules.
/// `{ check_datatype_and_pub_function_linking: false, check_datatype_layout:
/// false, check_friend_linking: false, check_private_entry_linking: _ }`:
/// Everything is broken. Need both a data migration and changes to dependent
/// modules.
...
pub struct Compatibility {
    ...
    /// `check_datatype_and_pub_function_linking`.
