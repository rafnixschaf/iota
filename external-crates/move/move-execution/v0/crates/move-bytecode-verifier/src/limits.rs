...
impl<'a> LimitsVerifier<'a> {
    ...
    fn verify_definitions(&self, config: &VerifierConfig) -> PartialVMResult<()> {
        ...
        if let Some(max_fields_in_struct) = config.max_fields_in_struct {
            ...
            // 1. Total number of fields in the enum (added across all variants) is less
            //    than the number of fields allowed in a struct.
            // 2. Total number of variants in the enum is less than the number of variants
            //    allowed in an enum.
