...
round number and a _certificate_ of availability signed by a quorum (2f+1) of validator stake. Each collection must also
contain hash pointers to a quorum of valid _certificate_s (from validators with 2f + 1 units of stake) from the previous
...
Signatures from 2f + 1 validators by stake form a _certificate_ of availability, which is then shared and potentially
