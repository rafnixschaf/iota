...
To convert an object into an immutable object, call the `public_freeze_object` function from the [transfer module](https://github.com/iotaledger/kinesis/blob/main/crates/iota-framework/packages/iota-framework/sources/transfer.move):
...
You can see this function's use in one of the [color_object example module](https://github.com/iotaledger/kinesis/blob/main/examples/move/color_object/sources/example.move) tests. The test creates a new (owned) `ColorObject`, then calls `public_freeze_object` to turn it into an immutable object.
