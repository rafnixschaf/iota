...
The `get_ip` function retrieves the public IP address of the node by making a GET request to `https://api.ipify.org`. If the request fails, it returns the `UNKNOWN_STRING`.
