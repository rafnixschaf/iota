...
Analytics (GA). It uses the GA4 Measurement Protocol that allows backends to send custom event data, to track node
...
It retrieves the current git revision using `env!("CARGO_PKG_VERSION")`, the
...
?format=json`. If the request fails, it returns the `UNKNOWN_STRING`.
After all data is retrieved, it constructs a `TelemetryEvent` and wraps it into
