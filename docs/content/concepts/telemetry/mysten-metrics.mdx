...
In order to manage Prometheus registries with their metrics more easily, the crate provides a `RegistryService` struct with a default registry and a collection of additional registries identified by unique UUIDs:
...
In order to expose the Prometheus metrics, the `RegistryService` is exposed by the `start_prometheus_server` function, which starts a HTTP server with `axum` and serves the metrics by the `/metrics` endpoint from the registries.
