...
pending futures, channel sizes, and scope activities:
...
Macros like `monitored_future!` and `spawn_monitored_task!` wrap and monitor futures and tasks, while updating metrics
accordingly.
...
For example, the `consensus_handler` module uses monitored scopes to track the duration and number of iterations of
