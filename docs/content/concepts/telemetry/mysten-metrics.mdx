...
In order to expose the Prometheus metrics, the `RegistryService` gets exposed by the `start_prometheus_server` function, which starts `axum` HTTP server and serves the metrics by the `/metrics` endpoint from the registries.
