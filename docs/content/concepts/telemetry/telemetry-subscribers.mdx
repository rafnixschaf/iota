...
The `telemetry-subscribers` crate is used for logging and tracing of the node.
It provides a tracing subscriber with flexible options to monitor the nodes performance and behavior.
...
The `TelemetryConfig` allows to set various configuration options, such as OpenTelemetry tracing, outputting JSON logs,
...
a name describing the operation, timing information (start and end), attributes (key-value pairs) for extra context or
...
debugging or integrate with Prometheus for measuring span latencies. Each layer layers will be enabled or disabled based
on the configuration.
If OTLP tracing is enabled, and `OpenTelemetryLayer` will be set up for tracing to either a file or an OTLP endpoint
...
To enable Prometheus monitoring, a `prometheus::Registry` must be passed to the `TelemetryConfig` using
the `with_prom_registry` function.
For example, in the `iota-node` it is set up as follows:
...
Additionally, in the main of `iota-node` a push task is started that pushes metrics regularly to the Prometheus service.
...
Every 60 seconds, this task will gather all metrics from the `mysten-metrics::RegistryService`,
assigns the current timestamp to each metric, encodes the metrics into the Protobuf format including compression and
sends the compressed metrics data via an HTTP POST request to the Prometheus service.
