...
IOTA supports the following API operations related to transferring IOTA between addresses:
 ...
 - [unsafe_pay](/iota-api-ref#unsafe_pay)
   This method is similar to `unsafe_payIota`, but it accepts any kind of coin or token instead of only IOTA (for example, SMR). You must include a IOTA gas token, and all of the coins or tokens must be the same type.
...
Refer to [IOTA Signatures](../cryptography/explanations/transaction-auth/signatures.mdx) for more details on signature validity requirements.
