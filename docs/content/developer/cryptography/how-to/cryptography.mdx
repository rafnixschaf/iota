...
Move contracts in IOTA support verifications for several on-chain signature schemes. Not all signatures supported in on-chain verification are supported as user signature verification. See [Signatures](../explanations/transaction-auth/signatures.mdx) for valid signature schemes for transaction authorization.

Go to [IOTA On-Chain Signatures Verification in Move](signing.mdx).
...
Go to [ECVRF](ecvrf.mdx).

## Related links

- [Cryptography concepts](../explanations/cryptography.mdx): Before you use the guides, you might want to learn about the concepts behind the use of cryptography on IOTA.