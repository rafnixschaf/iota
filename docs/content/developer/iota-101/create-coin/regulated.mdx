...
The output also shows the three objects that the publisher now owns: `UpgradeCap` for [package upgrades](../iota-move-concepts/packages/upgrade.mdx), `TreasuryCap` for minting or burning coins, and the `DenyCap` for adding or removing addresses to or from the deny list for this coin.
