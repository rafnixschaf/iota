...
import questions from '/json/developer/iota-101/transactions/ptb/prog-txn-blocks.json';
...
- [Object model](../../objects/object-model.mdx)
- [Move Concepts](../../move-overview/move-overview.mdx)
...
For object inputs, the metadata needed differs depending on the type of [ownership of the object](../../objects/object-ownership/object-ownership.mdx). The data for the `ObjectArg` enum follows:
...
The command produces a single result type `iota::package::UpgradeReceipt` which provides proof for that upgrade. For more details on upgrades, see [Upgrading Packages](../../move-overview/package-upgrades/upgrade.mdx).
