...
The modules for interacting with these fields can be found at [`dynamic_field`](https://github.com/iotaledger/iota/tree/develop/crates/iota-framework/packages/iota-framework/sources/dynamic_field.move) and [`dynamic_object_field`](https://github.com/iotaledger/iota/tree/develop/crates/iota-framework/packages/iota-framework/sources/dynamic_object_field.move) respectively.
...
```move
use iota::dynamic_object_field as ofield;
use iota::{object, transfer, tx_context};
...
IOTA provides `Table` and `Bag` collections built using dynamic fields, but with additional support to count the number of entries they contain to protect against accidental deletion when non-empty. To learn more, see [Tables and Bags](tables-bags.mdx).
