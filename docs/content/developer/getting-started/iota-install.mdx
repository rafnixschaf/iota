...
1. In the right pane, find the **Releases** section.
    ...
    ![IOTA releases in GitHub](/img/developer/getting-started/iota-install/releases.png)
...
1. Open the expanded folder and double-click the appropriate binary to install, beginning with iota-`<OS>`-`<ARCHITECTURE>`:

    - iota-faucet-`<OS>`-`<ARCHITECTURE>`: Local faucet to mint coins on local network.
    - iota-indexer-`<OS>`-`<ARCHITECTURE>`: An indexer for a local IOTA network.
    - iota-`<OS>`-`<ARCHITECTURE>`: Main IOTA binary.
    - iota-node-`<OS>`-`<ARCHITECTURE>`: Run a local node.
    - iota-test-validator-`<OS>`-`<ARCHITECTURE>`: Run test validators on a local network for development.
    - iota-tool-`<OS>`-`<ARCHITECTURE>`: Provides utilities for IOTA.

    The IOTA binary takes several minutes to download and install files, so make sure you allocate enough time for it to complete. Actual time to complete depends on your network connection and computer specifications.


### Confirm the installation {#confirm-the-installation}

To confirm that IOTA installed correctly, type `iota` and press Enter. You should see a message about the IOTA version installed and help for using IOTA commands.
...
You can also download the [source code](./iota-environment) to have local access to files.
...
## Install IOTA binaries from source {#install-iota-binaries-from-source}
...
- [Build your first dApp](first-app/first-app.mdx) to start your on-chain journey.
