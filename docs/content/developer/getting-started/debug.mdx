...
## Implementing Debug in `my_module`

To demonstrate the use of the `std::debug` module, update your `my_module` by integrating debug statements. Specifically, modify the `new_sword` function to output the `forge` value both before and after incrementing `swords_created`. Incorporate a `print_stack_trace` as well, resulting in the following function:
