...
This is the definition of the so-called [One-Time-Witness](../iota-101/iota-move-concepts/one-time-witness.mdx). Together with the `init` function definition, this ensures that this `Coin` will only be created once and never more on this chain. 
