...
The output also shows the three objects that the publisher now owns: `UpgradeCap` for [package upgrades](../../../concepts/iota-move-concepts/packages/upgrade.mdx), `TreasuryCap` for minting or burning coins, and the `DenyCap` for adding or removing addresses to or from the deny list for this coin.
...
For all `Coin` functions available, see the Iota framework [`coin` module documentation](https://github.com/iotaledger/iota/blob/develop/crates/iota-framework/docs/iota-framework/coin.md).
...
- [Regulated Coin and Deny List](./loyalty.mdx): Example of how to create tokens that reward brand or service loyalty on the Iota network.