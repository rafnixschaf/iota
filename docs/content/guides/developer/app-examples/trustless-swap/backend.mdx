...
This is the first in a [three-part guide](../trustless-swap.mdx) on how to build a trustless atomic swap on Iota.
...
The `id.delete()` function call is used to delete the shared `Escrow` object. Previously, Move supported only the deletion of owned objects, but [shared-object deletion has since been enabled](https://github.com/iotaledger/iota/pull/16008).
