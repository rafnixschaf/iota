...
This is the third in a [three-part guide](../trustless-swap.mdx) on how to build a trustless atomic swap on Iota.
...
- Check out [Iota Typescript SDK](https://wiki.iota.org/references/ts-sdk/typescript) for basic usage on how to interact with Iota with Typescript.
- Check out [Iota dApp Kit](https://wiki.iota.org/references/ts-sdk/dapp-kit) to learn basic building blocks for developing a dApp in the Iota ecosystem with React.js.
...
- This project is bootstrapped through `pnpm create @iota/dapp`. Please check out [@iota/create-dapp](https://wiki.iota.org/references/ts-sdk/dapp-kit/create-dapp) for how to scaffold a React.js Iota dApp project quickly.
...
Use the `useIotaClient()` hook from `dApp Kit` to retrieve the Iota client instance configured in the [**Set up providers**](#set-up-providers) step. The `useSignTransaction()` function is another hook from `dApp kit` that helps to sign the transaction block using the currently connected wallet. It displays the UI for users to review and sign their transactions with their selected wallet. To execute a transaction block, the `executeTransaction()` on the Iota client instance of the Iota TypeScript SDK. Use `react-hot-toast` as another dependency to toast transaction status to users.
