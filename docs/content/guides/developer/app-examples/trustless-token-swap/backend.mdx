...
This is the first in a [three-part guide](../trustless-token-swap.mdx) on how to build a trustless atomic swap on Iota.
...
Let's create a Iota project using the terminal command `iota move new escrow`. Create a file in the sources directory named `shared.move`, and let's go through the code together.
...
The `object::delete` function call is used to delete the shared `Escrow` object. Previously, Move supported only the deletion of owned objects, but [shared-object deletion has since been enabled](https://github.com/iotaledger/kinesis/pull/16008).
