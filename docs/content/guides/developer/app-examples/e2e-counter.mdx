...
Next, use the [Iota Client CLI](references/cli/client.mdx) to generate a new Move package. If you have [Iota installed](guides/developer/getting-started/iota-install.mdx), the Iota CLI is on your system. Run the following command in your terminal or console:
...
...
[dependencies]
Iota = { git = "https://github.com/iotaledger/kinesis.git", subdir = "crates/iota-framework/packages/iota-framework", rev = "framework/devnet" }
...
```
...
First, import `TransactionBlock` from `@mysten/iota.js`, `COUNTER_PACKAGE_ID` from your constants.ts file created previously, and `useSignAndExecuteTransactionBlock` from `@mysten/dapp-kit`.
...
```ts
import { useSignAndExecuteTransactionBlock, useIotaClient } from '@mysten/dapp-kit';
