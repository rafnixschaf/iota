...
- Tooling: [Iota Typescript SDK](https://wiki.iota.org/references/ts-sdk/typescript) for basic usage on how to interact with Iota using Typescript.
- Tooling: [Iota dApp Kit](https://wiki.iota.org/references/ts-sdk/dapp-kit) to learn basic building blocks for developing a dApp in the Iota ecosystem with React.js.
- Tooling: [`@iota/dapp`](https://wiki.iota.org/references/ts-sdk/dapp-kit/create-dapp), used within this project to quickly scaffold a React-based Iota dApp.
...
First, import `Transaction` from `@iota/iota`, `COUNTER_PACKAGE_ID` from your constants.ts file created previously, and `useSignAndExecuteTransaction` from `@iota/dapp-kit`.
...
You now have a functional component that can create a new `Counter` object, and because the `showRawEffects` and `showEffects` options are toggled on, the transaction results ensure finality. The `IotaClient` is used to set this up.
...
Your counter app is now ready to count. To learn more about dApp Kit, check out the [dApp Kit docs](https://wiki.iota.org/references/ts-sdk/dapp-kit).
