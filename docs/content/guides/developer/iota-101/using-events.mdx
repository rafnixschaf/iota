...
To create the event subscription, you can use a basic Node.js app. You need the Iota TypeScript SDK, so install the module using `npm install @mysten/iota.js` at the root of your project. In your TypeScript code, import `JsonRpcProvider` and a connection from the library.

```ts
import { JsonRpcProvider, testnetConnection } from '@mysten/iota.js';
