...
# Overview
A transaction on IOTA takes a payment to execute. The payment, also known as gas, is a list of `0x2::coin::Coin<0x2::iota::IOTA>` objects, and paid to IOTA validators to secure the network. Although gas is a critical piece in IOTA tokenomics, it sometimes adds challenges when new users start to navigate on IOTA, especially for web 2.0 users.
