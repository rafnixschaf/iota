...
1. Generate an ephemeral key pair. Follow the same process as you would generating a key pair in a traditional wallet. See [Iota SDK](https://wiki.iota.org/references/ts-sdk/typescript/cryptography/keypairs) for details.
...
First, sign the transaction bytes with the ephemeral private key using the key pair generated previously. This is the same as [traditional KeyPair signing](https://wiki.iota.org/references/ts-sdk/typescript/cryptography/keypairs). Make sure that the transaction `sender ` is also defined.
