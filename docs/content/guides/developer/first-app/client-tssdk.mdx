...
The Iota TypeScript SDK (@iota/iota) provides all the low-level functionality needed to interact with Iota ecosystem from TypeScript. You can use it in any TypeScript or JavaScript project, including web apps, Node.js apps, or mobile apps written with tools like React Native that support TypeScript.

For more information on the Iota TypeScript SDK, see the [Iota TypeScript SDK documentation](https://wiki.iota.org/references/ts-sdk/typescript).

## What is dApp Kit?

dApp Kit (@iota/dapp-kit) is a collection of React hooks, components, and utilities that make building dApps on Iota straightforward. For more information on dApp Kit, see the [dApp Kit documentation](https://wiki.iota.org/references/ts-sdk/dapp-kit).
...
```bash npm2yarn
npm install @iota/iota @iota/dapp-kit @tanstack/react-query
```
...
Next, set up the `IotaClientProvider`. This `Provider` delivers a `IotaClient` instance from `@iota/iota` to all the hooks in dApp Kit. This provider manages which network dApp Kit connects to, and can accept configuration for multiple networks. This exercise connects to `devnet`.
