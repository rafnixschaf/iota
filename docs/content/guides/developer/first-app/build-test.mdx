...
Update your `my_module.move` file to include a function callable from Iota that implements `sword` creation. With this in place, you can then add a multi-transaction test that uses the `test_scenario` module to test these new capabilities. Put this functions after the accessors (Part 5 in comments).
...
- No return values
- Private visibility
- Optionally, the parameter list starts by accepting the module's [one-time witness by value](../../../concepts/iota-move-concepts/one-time-witness.mdx)
