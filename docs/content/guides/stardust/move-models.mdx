...
genesis of the network.
 ...
 - The following modules are present in the package:
    - [`basic_output`](../../references/framework/stardust/basic_output): Defines how a basic output _container_ is modeled.
    - [`alias_output`](../../references/framework/stardust/alias_output): Defines how an alias output _container_ is modeled.
    - [`alias`](../../references/framework/stardust/alias): Defines how an alias _asset_ is modeled.
    - [`nft_output`](../../references/framework/stardust/nft_output): Defines how an NFT output _container_ is modeled.
    - [`nft`](../../references/framework/stardust/nft): Defines how an NFT _asset_ is modeled.
    - [`address_unlock_condition`](../../references/framework/stardust/address_unlock_condition): Defines how an address unlock condition is modeled.
    - [`expiration_unlock_condition`](../../references/framework/stardust/expiration_unlock_condition): Defines how an expiration unlock condition is modeled.
    - [`timelock_unlock_condition`](../../references/framework/stardust/timelock_unlock_condition): Defines how a timelock unlock condition is modeled.
    - [`irc27`](../../references/framework/stardust/irc27): Defines how an IRC-27 metadata is modeled.
    - [`utilities`](../../references/framework/stardust/utilities): Contains utility functions for the stardust package, primarily to help with extracting assets from containers.
 - There is no dedicated module for foundries and native tokens, since they are converted to [move-native tokens](../developer/evm-to-move/creating-token)
...
defines [`Coin`](../../references/framework/iota-framework/coin#struct-coin) as the primary object type to use by users to hold
tokens.
...
to hold balances of tokens, furthermore the [`CoinManager`](../../references/framework/iota-framework/coin_manager.md)
...
A native token and its foundry from the stardust ledger therefore translate to the following in the move ledger:
 - A published move package that defines the currency type ([one-time witness](../../concepts/iota-move-concepts/one-time-witness))
   for the native token. The template that is used for every native token package can be found [here](https://github.com/iotaledger/iota/blob/develop/crates/iota-genesis-builder/src/stardust/native_token/package_template/sources/native_token_template.move).
 ...
 - `Balance<TOKEN-SYMBOL>` values that are either stored inside `Coin<TOKEN-SYMBOL>` object or inside the [stardust containers](#assets-and-their-containers).
...
The Stardust Move Package preserve the Alias concept and models it as a separate object in the move ledger:
...
A more detailed description on the process can be found in [Advanced Topics](./advanced).
...
functionality in the new move framework](../../concepts/transfers/transfers/transfer-to-object#receiving-objects) that
...
rules](../../concepts/transfers/custom-rules). The only way to interact with the containers is to call `extract_assets()`
...
object](../../concepts/object-ownership/address-owned).
 ...
 - [Addresses and Keys](./addresses) describe in more detail how to convert the legacy addresses to move addresses.
...
The move model [`StorageDepositReturnUnlockCondition`](../../references/framework/stardust/storage_deposit_return_unlock_condition#struct-storagedepositreturnunlockcondition)
is added to the containers during the migration process where applicable. The [`unlock()`](../../references/framework/stardust/storage_deposit_return_unlock_condition#function-unlock)
...
locks funds until a certain time has passed. It is modeled in move via the [`TimelockUnlockCondition`](../../references/framework/stardust/timelock_unlock_condition#struct-timelockunlockcondition)
that is added to the containers during the migration process where applicable. The [`unlock()`](../../references/framework/stardust/timelock_unlock_condition#function-unlock)
...
[`Timelock<Balance<IOTA>>`](../../references/framework/timelock/timelock#resource-timelock).
  - `Timelock` objects can be staked to validators of the network to earn rewards. The functionality is provided by the
     [timelock::timelocked_staking](../../references/framework/timelock/timelocked_staking) module.
  - The [timelock](../../references/framework/timelock) package and it's utilities can be used by anyone to create their own timelocked assets.
...
In move, the expiration unlock condition is modeled via the [`ExpirationUnlockCondition`](../../references/framework/stardust/expiration_unlock_condition#struct-expirationunlockcondition).
The [`unlock()`](../../references/framework/stardust/expiration_unlock_condition#function-unlock) function is called when assets are extracted from the container,
...
migrated to [Shared Objects](http://localhost:3000/concepts/object-ownership/shared). As opposed to owned objects, shared objects
...
```
 ...
 - The unlock conditions are all defined as [`Options`](../../references/framework/move-stdlib/options) since not all outputs have them.
...
The are two ways to interact with `BasicOutputs` in move:
1. Call the [`extract_assets()`](../../references/framework/stardust/basic_output#function-extract_assets) function on
  ...
  of the base token balance and from the native token balances extracted from the `Bag`. The [Claiming Stardust Assets](./claiming)
  guide explains in detail what is the recommended way to deal with these results.

2. Receive them on an `ObjectID` address. The [Receiving Objects](../../concepts/transfers/transfers/transfer-to-object#receiving-objects)
  ...
  that were locked in the legacy network to `Alias` or `Nft` addreses:
   - [`unlock_alias_address_owned_basic()`](../../references/framework/stardust/address_unlock_condition#function-unlock_alias_address_owned_basic)
   - [`unlock_nft_address_owned_basic()`](../../references/framework/stardust/address_unlock_condition#function-unlock_nft_address_owned_basic)
...
```
 ...
 - This `Alias` object owns all other containers or `Coins` that were owned by the `Alias Address` in stardust. One can
   receive them via the following functions:
    -  [`unlock_alias_address_owned_basic()`](../../references/framework/stardust/address_unlock_condition#function-unlock_alias_address_owned_basic)
    -  [`unlock_alias_address_owned_alias()`](../../references/framework/stardust/address_unlock_condition#function-unlock_alias_address_owned_alias)
    -  [`unlock_alias_address_owned_nft()`](../../references/framework/stardust/address_unlock_condition#function-unlock_alias_address_owned_nft)
...
```
 ...
 - This `Nft` object owns all other containers or `Coins` that were owned by the `Nft Address` in stardust. One can
   receive them via the following functions:
    -  [`unlock_nft_address_owned_basic()`](../../references/framework/stardust/address_unlock_condition#function-unlock_nft_address_owned_basic)
    -  [`unlock_nft_address_owned_alias()`](../../references/framework/stardust/address_unlock_condition#function-unlock_nft_address_owned_alias)
    -  [`unlock_nft_address_owned_nft()`](../../references/framework/stardust/address_unlock_condition#function-unlock_nft_address_owned_nft)
    - For coins, use [transfer::public_receive()](../../references/framework/iota-framework/transfer#function-public_receive).
 - The immutable metadata field has been standardized as [`Irc27Metadata`](../../references/framework/stardust/irc27#struct-irc27metadata),
...
|---|---|---|
| Basic Output with IOTA/SMR only | [`Coin<IOTA/SMR>`](../../references/framework/iota-framework/coin#resource-coin)  | ready to be used, no need to extract any assets |
| Basic Output with IOTA/SMR and native tokens | [`Coin<IOTA/SMR>`](../../references/framework/iota-framework/coin#resource-coin) and one or more [`Coin<NATIVE_TOKEN>`](../../references/framework/iota-framework/coin#resource-coin)    | ready to be used, no need to extract any assets |
...
|  [Vested Rewards From the Stardust Upgrade](https://github.com/iotaledger/new_supply) | [`Timelock<Balance<IOTA>>`](../../references/timelock/timelock#resource-timelock) objects.   | -  |
