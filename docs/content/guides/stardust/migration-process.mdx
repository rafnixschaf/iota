...
After this process, the minted coin sits on the `0x0` address with the entire minted supply. When other output types that 
contain a balance of _this_ native token are migrated, then that balance is split off of the minted coin into a new
`Coin` object, which is then owned by the migrated output. If a non-zero balance remains on
the minted coin by the end of this process, it is left at the zero address. This means they were burned in Stardust and, 
therefore, are effectively also burned on the Move ledger, as no one controls the `0x0` address.
...
[`iota::transfer::receive`](../../references/framework/iota-framework/transfer.md#function-receive). There are
...
You can examine the `convert_immutable_metadata` function in the migration code for more
details.
