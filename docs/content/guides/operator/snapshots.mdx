...
When you restore a Full node from a snapshot, write it to the path `/opt/iota/db/authorities_db/full_node_db/live`. When restoring a Validator node, you can shorten the database destination to `/opt/iota/db/authorities_db/live`.
