...
IOTA Rebase introduces the Move-based object ledger, which necessitates the migration of assets and tokens from the Stardust
...
With the introduction of the object-based Move ledger in IOTA Rebase, transactions no longer consume and create outputs,
...
**The IOTA Rebase network will be started with a ledger state where all previous outputs, furthermore all tokens and assets held
 by them, are automatically converted into move objects, coins and balances.**

## What Happens To Shimmer?

The Shimmer network state is migrated to the IOTA Rebase network, using the very same move models as the IOTA Stardust network.
The SMR token, and all native tokens and assets on the Shimmer network, live on in the IOTA Rebase network, under the same
addresses.
...
are part of the move genesis that IOTA Rebase is started with.
...
The process is as follows:
 ...
 5. The genesis blob is published and the IOTA Rebase network is started with it.
 6. ShimmerEVM and IOTAEVM are started with a new node software that connects to the IOTA Rebase network.
 7. IOTA, SMR and stardust native tokens can be immediately used on the IOTA Rebase network, however more complex asset
...
IOTA Rebase is a major change in the technology stack with introduction of move smart contracts and the object ledger.
All previous IOTA and Shimmer integrations are broken and have to be reimplemented via the new APIs and SDKs.

## What Does Not Break?

**IOTA Rebase uses the same address scheme as IOTA/SMR Stardust, so all addresses and keys are still valid, there is no
...
find their assets on the IOTA Rebase network. Should it be necessary based on the assets of the user, a web application
