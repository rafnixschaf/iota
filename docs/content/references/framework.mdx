...
You can find the source code for these Move modules in the [crates/iota-framework/packages](https://github.com/iotaledger/kinesis/tree/main/crates/iota-framework/packages) directory in the `iota` repository on GitHub. As previously mentioned, the comments included in the code provide context for the logic defined.

## Crate documentation

You can review the raw `cargo doc` output of the following documentation in the `iota` repository. The .md files are located in the `crates/iota-framework/docs` directory. Online, they are located at https://github.com/iotaledger/kinesis/tree/main/crates/iota-framework/docs.
