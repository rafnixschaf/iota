...
Cloning the documentation locally is recommended when you are creating larger, more significant changes to the docs. See [IOTA Environment Setup](/developer/getting-started/iota-environment.mdx) for instructions on forking the IOTA repository, if necessary. The documentation is in the `docs/content` directory.
