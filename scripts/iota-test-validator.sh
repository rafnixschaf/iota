...
echo "iota-test-validator binary has been deprecated in favor of iota start, which is a more powerful command that allows you to start the local network with more options.
This script offers backward compatibiltiy, but ideally, you should migrate to iota start instead. Use iota start --help to see all the flags and options. 

To recreate the exact basic functionality of iota-test-validator, you must use the following options:
...
# Basic command that replicates the command line arguments of iota-test-validator
cmd="iota start --with-faucet --force-regenesis"
