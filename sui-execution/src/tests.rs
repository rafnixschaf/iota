...
#[test]
/// Make sure that all accesses to execution layer crates in the `sui-node` and
/// `sui-replay` crates go via the `sui-execution` crate (in other words, the
/// `sui-execution` crate dominates execution layer crates in the dependency
/// graphs of `sui-node` and `sui-replay`).
///
