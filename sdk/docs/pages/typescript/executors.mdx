...
import { SerialTransactionExecutor } from '@iota/iota/transactions';
...
-   `coinBatchSize`: The maximum number of new coins to create when refilling the gas pool
    (default 20)
-   `initialCoinBalance`: The balance of new coins created for the gas pool in NANOS (default
    `200_000_000n`),
...
-   `sourceCoins`: An array of coins to use to create the gas pool, defaults to using all coins
    owned by the signer.
-   `epochBoundaryWindow` Time to wait before/after the expected epoch boundary before re-fetching
...
```ts
import { getFullnodeUrl, IotaClient } from '@iota/iota/client';
import { ParallelTransactionExecutor } from '@iota/iota/transactions';
