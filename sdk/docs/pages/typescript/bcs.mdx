...
The `@iota/iota/bcs` package extends `@iota/bcs` with Iota specific scheme definitions.

To learn more about using BCS see the [BCS documentation](/bcs).

the `bcs` export of `@iota/iota/bcs` contains all the same exports as `bcs` from `@iota/bcs` plus
