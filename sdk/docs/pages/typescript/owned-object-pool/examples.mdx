...
Assume that you have a service that needs to make payments of size 50000000 MICROS to multiple
...
import type { IotaObjectRef, IotaObjectResponse } from '@mysten/iota.js/client/';
...
for (let i = 0; i < 10; i++) {
	...
	promises.push(
		eshandler.execute(
			txb,
			client,
			new DefaultSplitStrategy(300000000), // Each pool contains coins with a total balance of 300000000 MICROS
		),
	);
}
...
[`move_examples/nft_app` example](https://github.com/MystenLabs/Iota_Owned_Object_Pools/tree/main/move_examples/nft_app).
Notice how multiple `AdminCaps` are created in
[`genesis.move`](https://github.com/MystenLabs/Iota_Owned_Object_Pools/blob/main/move_examples/nft_app/sources/genesis.move):
