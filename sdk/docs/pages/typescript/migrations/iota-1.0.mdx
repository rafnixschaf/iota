...
- [A new GraphQL Client for the newly released Iota GraphQL API](../graphql)
...
The majority of changes in this release are in the `@iota/iota` package, but the changes to API
...
## Changes to `@iota/iota`

As part of the 1.0 release the `@iota/iota-sdk` package has been renamed to `@iota/iota`

To upgrade to the new version, uninstall the old package and install the new one:

```sh npm2yarn
npm uninstall @iota/iota-sdk
...
```diff
- import { IotaClient } from '@iota/iota-sdk'
+ import { IotaClient } from '@iota/iota'
```
...
coming to other parts of the IOTA ecosystem in the future.
...
- `iotaClient.waitForTransactionBlock({ transactionBlock })` ->
  `iotaClient.waitForTransaction({ transaction })`
- `iotaClient.signAndExecuteTransactionBlock({ transactionBlock })` ->
  `iotaClient.signAndExecuteTransaction({ transaction })`

There are a few methods on IotaClient that retain the `TransactionBlock` name because they correspond
to specific JSON RPC calls. The following methods are unchanged:

- `iotaClient.getTransactionBlock`
- `iotaClient.queryTransactionBlocks`
- `iotaClient.executeTransactionBlock`
- `iotaClient.dryRunTransactionBlock`
- `iotaClient.devInspectTransactionBlock`
...
```diff
- import { Transactions } from '@iota/iota/transactions'
+ import { Commands } from '@iota/iota/transactions'
...
- [`TransactionObjectArgument`](/typedoc/types/_iota_iota.transactions.TransactionObjectArgument.html)
  shape changed
- [`TransactionResult`](/typedoc/types/_iota_iota.transactions.TransactionResult.html) shape
  changed
- [`TransactionObjectInput`](/typedoc/types/_iota_iota.transactions.TransactionObjectInput.html)
  shape changed
- [`TransactionArgument`](/typedoc/types/_iota_iota.transactions.TransactionArgument.html) shape
  changed
- [`TransactionBlockInput`](/typedoc/types/_iota_iota.transactions.TransactionBlockInput.html)
  shape changed
...
The `publicKeyFromIotaBytes` has been moved to `@iota/iota/verify`

```diff
- import { publicKeyFromIotaBytes } from '@iota/iota/multisig'
...
We previously deprecated the `generic` helper exported from `@iota/bcs`. This helper has now been
entirely removed.
...
`iotaClient.executeTransactionBlock` for returning additional data.
...
`showRawEffects` to true if using `iotaClient.executeTransactionBlock`, or by returning an object
...
```diff
- import { getSentTransactionBlocksWithLinks, ZkSendLinkBuilder } from '@iota/zksend'
+ import { getSentTransactionsWithLinks, ZkSendLinkBuilder } from '@iota/zksend'
...
[wallet-standard documentation](https://wiki.iota.org/standards/wallet-standard).

The old `iota:signTransactionBlock` and `iota:signAndExecuteTransactionBlock` methods have been
deprecated in favor of the new `iota:signTransaction` and `iota:signAndExecuteTransaction` methods,
...
```ts
import { signAndExecuteTransaction, signTransaction } from '@iota/wallet-standard';
