...
To establish a connection to a network, import `IotaClient` from `@iota/iota/client` and pass the
...
[the Local Network guide](https://wiki.iota.org/guides/developer/getting-started/local-network) for
more information.
...
[RPC documentation](https://wiki.iota.org/references/iota-api).
...
```typescript
import { getFullnodeUrl, IotaClient, IotaHTTPTransport } from '@iota/iota/client';
