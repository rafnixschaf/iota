...
- `signAndExecuteTransactionBlock` - This method was not deprecated, but is now part of `IOTAClient`.
...
- The `superstruct` type definitions for `JsonRPCProvider` types are replaced with generated types
  exported from `@iota/iota.js/client`. The new type definitions are pure TypeScript types that you
...
- [`@iota/iota.js/cryptography`](/typescript/cryptography/keypairs) - Shared types and classes for
  cryptography.
