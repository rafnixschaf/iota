...
-   `defaultNetwork`: The name of the network to use by default when using the `SuiClientProvider`
...
-   `createClient`: A callback when a new `SuiClient` is created (for example, when the active
