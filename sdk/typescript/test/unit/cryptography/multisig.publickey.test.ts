...
describe('Publickey', () => {
    ...
    it('`toRawBytes()` should return correct array representation', async () => {
        ...
        expect(multiSigPublicKey.toRawBytes()).toEqual(
            new Uint8Array([
                3, 0, 90, 226, 32, 180, 178, 246, 94, 151, 124, 18, 237, 230, 21, 121, 255, 81, 112,
                182, 194, 44, 0, 97, 104, 195, 123, 94, 124, 97, 175, 1, 128, 131, 1, 1, 2, 29, 21,
                35, 7, 198, 183, 43, 14, 208, 65, 139, 14, 112, 205, 128, 231, 245, 41, 91, 141,
                134, 245, 114, 45, 63, 82, 19, 251, 210, 57, 79, 54, 2, 2, 2, 39, 50, 43, 58, 137,
                26, 10, 40, 13, 107, 193, 251, 44, 187, 35, 210, 143, 84, 144, 111, 214, 64, 127,
                95, 116, 31, 109, 239, 87, 98, 96, 154, 3, 3, 0,
            ]),
        );
    });
    ...
    it('`combinePartialSignatures()` should combine with different signatures into a single multisig correctly', async () => {
        ...
        expect(decoded).toEqual({
            ...
            sigs: [
                {
                    ED25519: Array.from(
                        parseSerializedSignature((await k1.signPersonalMessage(data)).signature)
                            .signature!,
                    ),
                },
                {
                    Secp256k1: Array.from(
                        parseSerializedSignature((await k2.signPersonalMessage(data)).signature)
                            .signature!,
                    ),
                },
            ],
