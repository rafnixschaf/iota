...
- 0259aec82: Removed dependency on @open-rpc/client-js and replaced it with standard fetch and WebSocket based APIs
  ...
  ```typescript
  import { getFullnodeUrl, IotaClient, IotaHTTPTransport } from '@mysten/iota.js/client';
...
- fd8589806: Remove all previously deprecated exports from @mysten/iota.js
...
- ba8e3b857: Rename TransactionBlock generated type in @mysten/iota.js/client to IotaTransactionBlock to avoid conflicting names in exports
...
- a503cad34: Add exports to `@mysten/iota.js/client` for rpc method params
...
- cc6441f46: The Iota TS SDK has been broken up into a set of modular exports, and all exports from the root of
  ...
  - `@mysten/iota.js/client` - A client for interacting with Iota RPC nodes.
  - `@mysten/iota.js/bcs` - A BCS builder with pre-defined types for Iota.
  - `@mysten/iota.js/transaction` - Utilities for building and interacting with transactions.
  - `@mysten/iota.js/keypairs/*` - Modular exports for specific KeyPair implementations.
  - `@mysten/iota.js/verify` - Methods for verifying transactions and messages.
  - `@mysten/iota.js/cryptography` - Shared types and classes for cryptography.
  - `@mysten/iota.js/multisig` - Utilities for working with multisig signatures.
  - `@mysten/iota.js/utils` - Utilities for formatting and parsing various Iota types.
  - `@mysten/iota.js/faucet`- Methods for requesting iota from a faucet.
  ...
  - The `superstruct` type definitions for types used by JsonRPCProvider are being replaced with
    generated types exported from `@mysten/iota.js/client`. The new type definitions are pure
  ...
  [Github Discussion thread](https://github.com/iotaledger/kinesis/discussions/13150) to track those
  use-cases.
  ...
  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@mysten/iota.js';
  ...
  a method `@mysten/iota.js/faucet` to make these requests

  ```diff
  - import { JsonRpcProvider, devnetConnection } from '@mysten/iota.js';
  - const provider = new JsonRpcProvider(devnetConnection);
  + import { requestIotaFromFaucetV0, getFaucetHost } from '@mysten/iota.js/faucet';
...
- 001148443: Introduce new `@mysten/iota.js/faucet` export, which should be used for all faucet interactions. This deprecates the previous `requestIotaFromFaucet` APIs that existed on the `JsonRpcProvider` and `Signer` classes.
