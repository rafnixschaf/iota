...
export class ZkSendLink {
    ...
    async #loadBag({
        ...
        const itemIds: string[] | undefined = (this.bagObject as any)?.content?.fields?.value
            ?.fields?.item_ids.fields.contents;
        ...
        objectsResponse.forEach((object, i) => {
            ...
            if (
                ...
                if (object.data.content?.dataType === 'moveObject') {
                    const amount = BigInt(
                        (object.data.content.fields as Record<string, string>).balance,
                    );
    ...
    async #loadClaimedAssets() {
        ...
        const transfer = tx.transaction.data.transaction.transactions.findLast(
            (tx): tx is Extract<IotaTransaction, { TransferObjects: unknown }> =>
